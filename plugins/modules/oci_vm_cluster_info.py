...
"""Ansible module for retrieving OCI vm cluster information."""
...
DOCUMENTATION = r"""
---
module: oci_vm_cluster_info
short_description: Retrieve information about OCI vm clusters
description:
    - Retrieve details about one or more vm clusters in Oracle Cloud Infrastructure.
    - Use I(vm_cluster_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    vm_cluster_id:
        description:
            - The OCID of a specific vm cluster to retrieve.
...
EXAMPLES = r"""
- name: List all vm clusters in a compartment
  stevefulme1.oci_cloud.oci_vm_cluster_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific vm cluster by ID
  stevefulme1.oci_cloud.oci_vm_cluster_info:
    vm_cluster_id: "ocid1.vm_cluster.oc1..example"
  register: result
"""

RETURN = r"""
vm_clusters:
    description: List of vm cluster details.
    returned: always
    type: list
    elements: dict
"""
...
def list_resources(client, module):
    """List vm clusters in a compartment."""
...
def get_resource(client, module):
    """Get a single vm cluster by ID."""
    resource_id = module.params["vm_cluster_id"]
    try:
        response = client.get_vm_cluster(resource_id)
...
def main():
    ...
    if module.params.get("vm_cluster_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, vm_clusters=resources)
