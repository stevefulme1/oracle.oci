...
"""Ansible module for retrieving OCI service gateway information."""
...
DOCUMENTATION = r"""
---
module: oci_service_gateway_info
short_description: Retrieve information about OCI service gateways
description:
    - Retrieve details about one or more service gateways in Oracle Cloud Infrastructure.
    - Use I(service_gateway_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    service_gateway_id:
        description:
            - The OCID of a specific service gateway to retrieve.
...
EXAMPLES = r"""
- name: List all service gateways in a compartment
  stevefulme1.oci_cloud.oci_service_gateway_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific service gateway by ID
  stevefulme1.oci_cloud.oci_service_gateway_info:
    service_gateway_id: "ocid1.service_gateway.oc1..example"
  register: result
"""
...
def get_resource(client, module):
    """Get a single service gateway by ID."""
    resource_id = module.params["service_gateway_id"]
    try:
        response = client.get_service_gateway(resource_id)
...
def main():
    ...
    if module.params.get("service_gateway_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, service_gateways=resources)
