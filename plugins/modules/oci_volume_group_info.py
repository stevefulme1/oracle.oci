...
"""Ansible module for retrieving OCI volume group information."""
...
short_description: Retrieve information about OCI volume groups
description:
    - Retrieve details about one or more volume groups in Oracle Cloud Infrastructure.
    - Use I(volume_group_id) to get a single resource, or I(compartment_id) to list resources.
...
- name: List all volume groups in a compartment
  stevefulme1.oci_cloud.oci_volume_group_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific volume group by ID
  stevefulme1.oci_cloud.oci_volume_group_info:
    volume_group_id: "ocid1.volume_group.oc1..example"
  register: result
"""

RETURN = r"""
volume_groups:
    description: List of volume group details.
    returned: always
    type: list
    elements: dict
"""
...
def list_resources(client, module):
    """List volume groups in a compartment."""
...
def get_resource(client, module):
    """Get a single volume group by ID."""
    resource_id = module.params["volume_group_id"]
    try:
        response = client.get_volume_group(resource_id)
...
def main():
    ...
    if module.params.get("volume_group_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, volume_groups=resources)
