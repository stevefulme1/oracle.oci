...
"""Ansible module for retrieving OCI volume backup information."""
...
short_description: Retrieve information about OCI volume backups
description:
    - Retrieve details about one or more volume backups in Oracle Cloud Infrastructure.
    - Use I(volume_backup_id) to get a single resource, or I(compartment_id) to list resources.
...
- name: List all volume backups in a compartment
  stevefulme1.oci_cloud.oci_volume_backup_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific volume backup by ID
  stevefulme1.oci_cloud.oci_volume_backup_info:
    volume_backup_id: "ocid1.volume_backup.oc1..example"
  register: result
"""

RETURN = r"""
volume_backups:
    description: List of volume backup details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    """Get a single volume backup by ID."""
    resource_id = module.params["volume_backup_id"]
    try:
        response = client.get_volume_backup(resource_id)
...
def main():
    ...
    if module.params.get("volume_backup_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, volume_backups=resources)
