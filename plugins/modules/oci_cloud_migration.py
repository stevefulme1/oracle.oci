...
"""Ansible module for managing OCI Cloud Migrations."""
...
DOCUMENTATION = r"""
---
module: oci_cloud_migration
short_description: Manage Cloud Migrations in OCI
description:
    - Create, update, and delete Cloud Migrations in Oracle Cloud Infrastructure.
    - This module uses the OCI Python SDK C(oci.cloud_migrations.CloudMigrationsClient).
version_added: "1.0.0"
author:
    - Oracle (@oracle)
options:
...
RETURN = r"""
cloud_migration:
    description: Details of the cloud migration.
    returned: On success when state is present.
    type: dict
    sample:
        id: "ocid1.migration.oc1..example"
        display_name: "my-migration"
...
try:
    from oci.cloud_migrations import CloudMigrationsClient
...
def get_resource(client, resource_id):
    try:
        response = call_with_retry(client.get_migration, resource_id)
...
def find_resource(client, compartment_id, display_name):
    ...
    try:
        response = call_with_retry(
            client.list_migrations, compartment_id=compartment_id,
        )
...
def create_resource(module, client):
    ...
    resource = wait_for_resource(
        module, client.get_migration, resource.id, target_states=READY_STATES,
    )
    return resource
...
def update_resource(module, client, existing):
    ...
    response = call_with_retry(client.update_migration, existing.id, update_details)
    resource = response.data
    resource = wait_for_resource(
        module, client.get_migration, resource.id, target_states=READY_STATES,
    )
...
def delete_resource(module, client, existing):
    ...
    wait_for_resource(
        module, client.get_migration, existing.id, target_states=DEAD_STATES,
    )
...
def main():
    ...
    client = create_service_client(module, CloudMigrationsClient)
    params = module.params
    state = params["state"]

    existing = None
    if params.get("migration_id"):
        existing = get_resource(client, params["migration_id"])
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, cloud_migration=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, cloud_migration=to_dict(existing))
