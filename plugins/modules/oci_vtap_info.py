...
"""Ansible module for retrieving OCI vtap information."""
...
DOCUMENTATION = r"""
---
module: oci_vtap_info
short_description: Retrieve information about OCI vtaps
description:
    - Retrieve details about one or more vtaps in Oracle Cloud Infrastructure.
    - Use I(vtap_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    vtap_id:
        description:
            - The OCID of a specific vtap to retrieve.
...
EXAMPLES = r"""
- name: List all vtaps in a compartment
  stevefulme1.oci_cloud.oci_vtap_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific vtap by ID
  stevefulme1.oci_cloud.oci_vtap_info:
...
RETURN = r"""
vtaps:
    description: List of vtap details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    """Get a single vtap by ID."""
    resource_id = module.params["vtap_id"]
    try:
        response = client.get_vtap(resource_id)
...
def main():
    ...
    if module.params.get("vtap_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, vtaps=resources)
