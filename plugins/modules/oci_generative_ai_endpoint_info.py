...
"""Ansible module for retrieving OCI endpoint information."""
...
DOCUMENTATION = r"""
---
module: oci_generative_ai_endpoint_info
short_description: Retrieve information about OCI endpoints
description:
    - Retrieve details about one or more endpoints in Oracle Cloud Infrastructure.
    - Use I(endpoint_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    endpoint_id:
        description:
            - The OCID of a specific endpoint to retrieve.
...
EXAMPLES = r"""
- name: List all endpoints in a compartment
  stevefulme1.oci_cloud.oci_generative_ai_endpoint_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific endpoint by ID
  stevefulme1.oci_cloud.oci_generative_ai_endpoint_info:
...
def list_resources(client, module):
    """List endpoints in a compartment."""
...
def get_resource(client, module):
    ...
    resource_id = module.params["endpoint_id"]
    try:
        response = client.get_endpoint(resource_id)
...
def main():
    ...
    if module.params.get("endpoint_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, endpoints=resources)
