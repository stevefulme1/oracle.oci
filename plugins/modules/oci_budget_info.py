...
"""Ansible module for retrieving OCI budget information."""
...
DOCUMENTATION = r"""
---
module: oci_budget_info
short_description: Retrieve information about OCI budgets
description:
    - Retrieve details about one or more budgets in Oracle Cloud Infrastructure.
    - Use I(budget_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    budget_id:
        description:
            - The OCID of a specific budget to retrieve.
...
EXAMPLES = r"""
- name: List all budgets in a compartment
  stevefulme1.oci_cloud.oci_budget_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific budget by ID
  stevefulme1.oci_cloud.oci_budget_info:
...
RETURN = r"""
budgets:
    description: List of budget details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    ...
    resource_id = module.params["budget_id"]
    try:
        response = client.get_budget(resource_id)
...
def main():
    ...
    client = create_service_client(module, oci.budget.BudgetClient)

    if module.params.get("budget_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, budgets=resources)
