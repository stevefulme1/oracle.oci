...
"""Ansible module for retrieving OCI local peering gateway information."""
...
short_description: Retrieve information about OCI local peering gateways
description:
    - Retrieve details about one or more local peering gateways in Oracle Cloud Infrastructure.
    - Use I(local_peering_gateway_id) to get a single resource, or I(compartment_id) to list resources.
...
- name: List all local peering gateways in a compartment
  stevefulme1.oci_cloud.oci_local_peering_gateway_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific local peering gateway by ID
  stevefulme1.oci_cloud.oci_local_peering_gateway_info:
    local_peering_gateway_id: "ocid1.local_peering_gateway.oc1..example"
  register: result
"""

RETURN = r"""
local_peering_gateways:
    description: List of local peering gateway details.
    returned: always
    type: list
    elements: dict
"""
...
def list_resources(client, module):
    """List local peering gateways in a compartment."""
...
def get_resource(client, module):
    """Get a single local peering gateway by ID."""
    resource_id = module.params["local_peering_gateway_id"]
    try:
        response = client.get_local_peering_gateway(resource_id)
...
def main():
    ...
    module = AnsibleModule(
        ...
        required_one_of=[
            ("compartment_id", "local_peering_gateway_id"),
        ],
    )
    ...
    if module.params.get("local_peering_gateway_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, local_peering_gateways=resources)
