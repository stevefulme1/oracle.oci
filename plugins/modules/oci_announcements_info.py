...
"""Ansible module for retrieving OCI announcements information."""
...
RETURN = r"""
announcements:
    description: List of announcements details.
    returned: always
    type: list
    elements: dict
"""
...
def main():
    ...
    client = create_service_client(module, AnnouncementClient)
    params = module.params

    try:
        response = call_with_retry(client.list_announcements, compartment_id=params["compartment_id"])
        results = [to_dict(item) for item in response.data]
        module.exit_json(changed=False, announcements=results)
    except ServiceError as e:
        module.fail_json(msg=f"Failed to list announcements: {e.message}")
