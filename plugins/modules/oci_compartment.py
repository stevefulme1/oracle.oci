...
"""Ansible module for managing OCI compartments."""
...
description:
    - Create, update, and delete compartments in Oracle Cloud Infrastructure.
    - Compartments provide a logical grouping for organizing and isolating
      cloud resources.
version_added: "1.0.0"
...
options:
    parent_compartment_id:
        description:
            - The OCID of the parent compartment in which to create the compartment.
            - Required when creating a new compartment.
        type: str
    name:
        description:
            - The name of the compartment. Must be unique within the parent compartment.
            - Required when creating a new compartment.
        type: str
    description:
        description:
    ...
    compartment_id:
        description:
            - The OCID of the compartment to update or delete.
            - Required for update and delete operations.
        type: str
    state:
        description:
            - The desired state of the compartment.
        type: str
        default: present
...
RETURN = r"""
resource:
    description: The compartment details.
    returned: on success
    type: dict
    contains:
        id:
...
def get_existing_resource(client, module):
    """Return the existing compartment or None."""
    compartment_id = module.params.get("compartment_id")
    if compartment_id:
        try:
            return call_with_retry(client.get_compartment, compartment_id).data
        except ServiceError as e:
            if e.status == 404:
                return None
            raise

    # Try to find by name under parent compartment
    parent_id = module.params.get("parent_compartment_id")
    name = module.params.get("name")
    if parent_id and name:
        compartments = call_with_retry(
...
def create_resource(client, module):
    ...
    kwargs = dict(
        compartment_id=module.params["parent_compartment_id"],
    ...
    details = CreateCompartmentDetails(**kwargs)
    resource = call_with_retry(client.create_compartment, details).data
...
def update_resource(client, module, resource):
    ...
    details = UpdateCompartmentDetails(**kwargs)
    resource = call_with_retry(client.update_compartment, resource.id, details).data
...
def delete_resource(client, module, resource):
    ...
    call_with_retry(client.delete_compartment, resource.id)
    if module.params.get("wait", True):
        wait_for_resource(
            module,
            client.get_compartment,
