...
"""Ansible module for retrieving OCI queue information."""
...
DOCUMENTATION = r"""
---
module: oci_queue_info
short_description: Retrieve information about OCI queues
description:
    - Retrieve details about one or more queues in Oracle Cloud Infrastructure.
    - Use I(queue_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    queue_id:
        description:
            - The OCID of a specific queue to retrieve.
...
EXAMPLES = r"""
- name: List all queues in a compartment
  stevefulme1.oci_cloud.oci_queue_info:
...
RETURN = r"""
queues:
    description: List of queue details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    ...
    resource_id = module.params["queue_id"]
    try:
        response = client.get_queue(resource_id)
...
def main():
    ...
    client = create_service_client(module, oci.queue.QueueAdminClient)

    if module.params.get("queue_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, queues=resources)
