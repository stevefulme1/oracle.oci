...
"""Ansible module for retrieving OCI alarm information."""
...
DOCUMENTATION = r"""
---
module: oci_alarm_info
short_description: Retrieve information about OCI alarms
description:
    - Retrieve details about one or more alarms in Oracle Cloud Infrastructure.
    - Use I(alarm_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    alarm_id:
        description:
            - The OCID of a specific alarm to retrieve.
...
EXAMPLES = r    limit:
    ...
    max_results:
        ...
        default: 1000
"""
- name: List all alarms in a compartment
  stevefulme1.oci_cloud.oci_alarm_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific alarm by ID
  stevefulme1.oci_cloud.oci_alarm_info:
...
RETURN = r"""
alarms:
    description: List of alarm details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    ...
    resource_id = module.params["alarm_id"]
    try:
        response = client.get_alarm(resource_id)
...
def main():
    ...
    client = create_service_client(module, oci.monitoring.MonitoringClient)

    if module.params.get("alarm_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, alarms=resources)
