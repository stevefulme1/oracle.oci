...
"""Ansible module for retrieving OCI stream information."""
...
DOCUMENTATION = r"""
---
module: oci_streaming_info
short_description: Retrieve information about OCI streams
description:
    - Retrieve details about one or more streams in Oracle Cloud Infrastructure.
    - Use I(stream_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    stream_id:
        description:
            - The OCID of a specific stream to retrieve.
...
EXAMPLES = r"""
- name: List all streams in a compartment
  stevefulme1.oci_cloud.oci_streaming_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific stream by ID
  stevefulme1.oci_cloud.oci_streaming_info:
...
def get_resource(client, module):
    ...
    resource_id = module.params["stream_id"]
    try:
        response = client.get_stream(resource_id)
...
def main():
    ...
    client = create_service_client(module, oci.streaming.StreamAdminClient)

    if module.params.get("stream_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, streams=resources)
