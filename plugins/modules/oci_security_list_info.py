...
"""Ansible module for retrieving OCI security list information."""
...
short_description: Retrieve information about OCI security lists
description:
    - Retrieve details about one or more security lists in Oracle Cloud Infrastructure.
    - Use I(security_list_id) to get a single resource, or I(compartment_id) to list resources.
...
- name: List all security lists in a compartment
  stevefulme1.oci_cloud.oci_security_list_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific security list by ID
  stevefulme1.oci_cloud.oci_security_list_info:
    security_list_id: "ocid1.security_list.oc1..example"
  register: result
"""

RETURN = r"""
security_lists:
    description: List of security list details.
    returned: always
    type: list
    elements: dict
"""
...
def list_resources(client, module):
    """List security lists in a compartment."""
...
def get_resource(client, module):
    """Get a single security list by ID."""
    resource_id = module.params["security_list_id"]
    try:
        response = client.get_security_list(resource_id)
...
def main():
    ...
    module = AnsibleModule(
        ...
        required_one_of=[
            ("compartment_id", "security_list_id"),
        ],
    )
    ...
    if module.params.get("security_list_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, security_lists=resources)
