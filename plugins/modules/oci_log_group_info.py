...
"""Ansible module for retrieving OCI log group information."""
...
DOCUMENTATION = r"""
---
module: oci_log_group_info
short_description: Retrieve information about OCI log groups
description:
    - Retrieve details about one or more log groups in Oracle Cloud Infrastructure.
    - Use I(log_group_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    log_group_id:
        description:
            - The OCID of a specific log group to retrieve.
...
- name: Get a specific log group by ID
  stevefulme1.oci_cloud.oci_log_group_info:
    log_group_id: "ocid1.log_group.oc1..example"
  register: result
"""

RETURN = r"""
log_groups:
    description: List of log group details.
    returned: always
    type: list
    elements: dict
"""
...
def get_resource(client, module):
    """Get a single log group by ID."""
    resource_id = module.params["log_group_id"]
    try:
        response = client.get_log_group(resource_id)
...
def main():
    ...
    client = create_service_client(module, oci.logging.LoggingManagementClient)

    if module.params.get("log_group_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, log_groups=resources)
