...
"""Ansible module for retrieving OCI nat gateway information."""
...
DOCUMENTATION = r"""
---
module: oci_nat_gateway_info
short_description: Retrieve information about OCI nat gateways
description:
    - Retrieve details about one or more nat gateways in Oracle Cloud Infrastructure.
    - Use I(nat_gateway_id) to get a single resource, or I(compartment_id) to list resources.
...
- name: List all nat gateways in a compartment
  stevefulme1.oci_cloud.oci_nat_gateway_info:
    compartment_id: "ocid1.compartment.oc1..example"
  register: result

- name: Get a specific nat gateway by ID
  stevefulme1.oci_cloud.oci_nat_gateway_info:
    nat_gateway_id: "ocid1.nat_gateway.oc1..example"
  register: result
"""
...
def list_resources(client, module):
    """List nat gateways in a compartment."""
...
def get_resource(client, module):
    """Get a single nat gateway by ID."""
    resource_id = module.params["nat_gateway_id"]
    try:
        response = client.get_nat_gateway(resource_id)
...
def main():
    ...
    module.exit_json(changed=False, nat_gateways=resources)
