...
"""Ansible module for retrieving OCI dhcp options information."""
...
short_description: Retrieve information about OCI dhcp optionss
description:
    - Retrieve details about one or more dhcp optionss in Oracle Cloud Infrastructure.
    - Use I(dhcp_id) to get a single resource, or I(compartment_id) to list resources.
...
options:
    ...
    dhcp_id:
        description:
            - The OCID of a specific dhcp options to retrieve.
...
- name: Get a specific dhcp options by ID
  stevefulme1.oci_cloud.oci_dhcp_options_info:
    dhcp_id: "ocid1.dhcp_options.oc1..example"
  register: result
"""

RETURN = r"""
dhcp_optionss:
    description: List of dhcp options details.
    returned: always
    type: list
    elements: dict
"""
...
def list_resources(client, module):
    """List dhcp optionss in a compartment."""
...
def get_resource(client, module):
    """Get a single dhcp options by ID."""
    resource_id = module.params["dhcp_id"]
    try:
        response = client.get_dhcp_options(resource_id)
...
def main():
    ...
    if module.params.get("dhcp_id"):
        resources = get_resource(client, module)
    else:
        resources = list_resources(client, module)

    module.exit_json(changed=False, dhcp_optionss=resources)
