"""Unit tests for oracle.oci.oci_route_table_info module."""
...
import ansible_collections.oracle.oci.plugins.modules.oci_route_table_info as _target_module
if not hasattr(_target_module, "to_dict"):
    _target_module.to_dict = _real_to_dict


MODULE_PATH = "ansible_collections.oracle.oci.plugins.modules.oci_route_table_info"


def _mock_resource(display_name="test-resource", resource_id="ocid1.route_table.oc1..test"):
...
class TestGetOciRouteTableInfo:
    """Test retrieving a single route_table by ID."""

    def test_get_resource_by_id(self, info_module_args):
        """get_route_table() returns a single resource wrapped in a list."""
        mock_client = MagicMock()

        resource = _mock_resource()
        mock_response = MagicMock()
        mock_response.data = resource
        mock_client.get_route_table.return_value = mock_response

        info_module_args["route_table_id"] = "ocid1.route_table.oc1..test"

        module = MagicMock()
        module.params = info_module_args

        from ansible_collections.oracle.oci.plugins.modules.oci_route_table_info import get_resource
        result = get_resource(mock_client, module)

        mock_client.get_route_table.assert_called_once_with("ocid1.route_table.oc1..test")
        assert len(result) == 1
    ...
    def test_get_resource_404_returns_empty(self, info_module_args):
        ...
        info_module_args["route_table_id"] = "ocid1.route_table.oc1..gone"
    ...
    def test_get_resource_non_404_fails(self, info_module_args):
        ...
        info_module_args["route_table_id"] = "ocid1.route_table.oc1..bad"
...
class TestListOciRouteTableInfo:
    """Test listing route_tables by compartment."""

    def test_list_resources(self, info_module_args):
        """list_route_tables() returns resources in the compartment."""
        mock_client = MagicMock()
        ...
        with patch("oci.pagination.list_call_get_all_results", return_value=mock_pagination_response):
            from ansible_collections.oracle.oci.plugins.modules.oci_route_table_info import list_resources
