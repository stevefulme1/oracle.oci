"""Unit tests for oracle.oci.oci_alarm_info module."""
...
import ansible_collections.oracle.oci.plugins.modules.oci_alarm_info as _target_module
if not hasattr(_target_module, "to_dict"):
    _target_module.to_dict = _real_to_dict


MODULE_PATH = "ansible_collections.oracle.oci.plugins.modules.oci_alarm_info"


def _mock_resource(display_name="test-resource", resource_id="ocid1.alarm.oc1..test"):
...
class TestGetOciAlarmInfo:
    """Test retrieving a single alarm by ID."""

    def test_get_resource_by_id(self, info_module_args):
        """get_alarm() returns a single resource wrapped in a list."""
        mock_client = MagicMock()

        resource = _mock_resource()
        mock_response = MagicMock()
        mock_response.data = resource
        mock_client.get_alarm.return_value = mock_response

        info_module_args["alarm_id"] = "ocid1.alarm.oc1..test"

        module = MagicMock()
        module.params = info_module_args

        from ansible_collections.oracle.oci.plugins.modules.oci_alarm_info import get_resource
        result = get_resource(mock_client, module)

        mock_client.get_alarm.assert_called_once_with("ocid1.alarm.oc1..test")
        assert len(result) == 1
    ...
    def test_get_resource_404_returns_empty(self, info_module_args):
        ...
        info_module_args["alarm_id"] = "ocid1.alarm.oc1..gone"
    ...
    def test_get_resource_non_404_fails(self, info_module_args):
        ...
        info_module_args["alarm_id"] = "ocid1.alarm.oc1..bad"
...
class TestListOciAlarmInfo:
    """Test listing alarms by compartment."""

    def test_list_resources(self, info_module_args):
        """list_alarms() returns resources in the compartment."""
        mock_client = MagicMock()
        ...
        with patch("oci.pagination.list_call_get_all_results", return_value=mock_pagination_response):
            from ansible_collections.oracle.oci.plugins.modules.oci_alarm_info import list_resources
